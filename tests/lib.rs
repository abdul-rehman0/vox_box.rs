...
fn test_against_praat() {
    ...
    let mut samples: Vec<f64> = reader
        .samples::<i32>()
        .map(|sample| sample.unwrap() as f64 / (i32::MAX >> (32 - bits)) as f64)
        .collect();
    ...
    let _segment: Vec<f64> = samples[start..end].iter().map(|s| s - mean).collect();
    ...
    let mut complex_work =
        vec![Complex::new(0f64, 0.); vox_box::find_formants_complex_work_size(n_coeffs)];

    let mut formants: Vec<Resonance<f64>> = vox_box::MALE_FORMANT_ESTIMATES
        .iter()
        .map(|f| Resonance::new(*f, 1.0))
        .collect();
    vox_box::find_formants(
        &mut samples[..],
        sample_rate,
        resample_ratio,
        &mut resampled_buf[..],
...
fn test_formant_calculation() {
    ...
    for ((_idx, frame), _rms) in all_formants.iter().enumerate().zip(powers.iter()) {

...
use sample::{FromSample, Sample, ToSampleSlice};
...
pub fn interpolate_sinc<S: Sample>(
    y: &[S],
    offset: isize,
    nx: usize,
    x: S,
    ...
    if x > nx as f64 {
        return y[offset as usize + nx as usize - 1]
            .to_float_sample()
            .to_sample::<f64>();
    }
    if x < 0. {
        return y[0].to_float_sample().to_sample::<f64>();
    }
    if (x - nl as f64).abs() < 1.0e-10 {
        return y[offset as usize + nl].to_float_sample().to_sample::<f64>();
    };
    if (x - nr as f64).abs() < 1.0e-10 {
        return y[offset as usize + nr].to_float_sample().to_sample::<f64>();
    };
    ...
    for n in 0..=max_depth {
        ...
        result += {
            ...
            if lag_val >= y.len() as i32 {
                lag_val = y.len() as i32 - 1;
            }
            let r_lag = y[lag_val as usize].to_float_sample().to_sample::<f64>();
            let first = a.sin() / a;
...
fn brent_maximize<'a, S: Sample>(
    f: &dyn Fn(f64, &BrentParams<'a, S>) -> f64,
    bounds: (f64, f64),
...
pub fn improve_extremum<S: Sample + FromSample<f64>>(
    y: &[S],
    offset: isize,
    nx: usize,
    ixmid: f64,
    ...
    if ixmid == 0. {
        return (0., y[0].to_float_sample().to_sample::<f64>());
    }
    if ixmid >= nx as f64 {
        return (nx as f64, y[nx - 1].to_float_sample().to_sample::<f64>());
    }

    match interp {
        Interpolation::None => (0., y[0].to_float_sample().to_sample::<f64>()),
        Interpolation::Parabolic => {
            let diff = y[ixmid.floor() as usize + 1]
                .to_float_sample()
                .to_sample::<f64>()
                - y[ixmid.floor() as usize - 1]
                    .to_float_sample()
                    .to_sample::<f64>();
            let mid = y[ixmid.floor() as usize]
                .to_float_sample()
                .to_sample::<f64>();
            let dy = 0.5 * diff;
...
impl sample::window::Type for HanningLag {
    fn at_phase<S: Sample>(phase: S) -> S {
        ...
        ((one - phase.to_float_sample())
            * (two_thirds + (one_third * v.cos().to_sample()).to_sample::<S::Float>())
            + (one / pi_2) * v.sin().to_sample())
        .to_sample::<S>()
    }
}
    ...
    S: Sample,
{
    ...
    fn local_maxima(&self) -> Vec<(usize, S)> {
        self.windows(3)
            .enumerate()
            .filter(|x| x.1[0] < x.1[1] && x.1[2] < x.1[1])
            .map(|x| ((x.0 + 1), x.1[1]))
            .collect()
    }
}
    ...
    T: Float + FromPrimitive,
{
    ...
    fn pitch<W: LagType>(
        ...
        let window_lag = Window::<[S; 1], W::Lag>::new(self.len())
            .take(self.len())
            .map(|x| x.to_sample_slice()[0]);
        ...
        // TODO: remove allocation
        let mut maxima: Vec<Pitch<T>> = self_lag[0..brent_ixmax as usize]
            .local_maxima()
            .iter()
            .map(|x| {
                ...
                let n = (sample_rate / freq - T::from(offset).unwrap())
                    .to_f64()
                    .unwrap()
                    .to_sample::<S>();
                ...
                Pitch {
                    frequency: freq,
                    strength: T::from_f64(strn).unwrap(),
                }
            })
            .filter(|x| {
                ((x.frequency) == T::from_f64(0f64).unwrap())
                    || (x.frequency > min && x.frequency < max)
            })
            .map(|mut p| {
                let offset = -(brent_ixmax as isize) - 1;
                let nx = (brent_ixmax as isize - offset) as usize;
                let n = (sample_rate / p.frequency - T::from(offset).unwrap())
                    .to_f64()
                    .unwrap();
                let (mut xmid, mut ymid) =
                    improve_extremum(&self_lag[..], offset, nx, n, Interpolation::Sinc(700), true);
                xmid += offset as f64;
                if ymid > 1. {
                    ymid = 1. / ymid;
                }
    ...
    fn test_pitch() {
        ...
        for chunk in window::Windower::hanning(&vector[..], bin, hop) {
            ...
            let pitch = chunk_data
                .to_sample_slice()
                .pitch::<window::Hanning>(44100., 0.2, maxima, maxima, 100., 500.);
            println!("pitch: {:?}", pitch);
